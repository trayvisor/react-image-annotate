...
export default (function (state, action) {
  ...
  if (action.type === "ON_PRD_ADDED" && action.prd && action.prd !== "") {
    var oldRegionPrdList = state.regionPrdList;
    ...
    case "CHANGE_REGION":
      {
        ...
        if (oldRegion.prd !== action.region.prd) {
          state = saveToHistory(state, "Change Region Classification");
          var prdIndex = state.regionPrdList.indexOf(action.region.prd);
          ...
          case "DRAW_EXPANDING_LINE":
            {
              ...
              if (isMouseDown) {
                ...
                var _newState2 = setIn(state, [].concat(_toConsumableArray(pathToActiveImage), ["regions", _regionIndex9, "points"]), expandingLine.points.slice(0, -1).concat([_objectSpread({}, lastPoint, {
    ...
    case "MOUSE_DOWN":
      {
        ...
        var defaultRegionPrd = state.selectedPrd;
        ...
        var _prdIndex = (state.regionPrdList || []).indexOf(defaultRegionPrd);
